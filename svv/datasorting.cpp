...
void print_ohlc(int token, const OHLCData &ohlc, uint64_t epoch_second, uint64_t exchange_ordernumber1, uint64_t exchange_ordernumber2, char order_traded_type)
{
    ...
    // auto it = bsstick.find(exchange_ordernumber1);
    // auto it2 = bsstick.find(exchange_ordernumber2);
    // if (it != bsstick.end() && it2 != bsstick.end())
    if (order_traded_type == 'T')
    {
        // char stick_type;
        // if (it->second.time > it2->second.time)
        //     stick_type = 'B';
        // else
        //     stick_type = 'S';

        // std::string stick_type_colored;
        // if (stick_type == 'B') {
        //     stick_type_colored = std::string(COLOR_GREEN) + "B" + COLOR_RESET;
        // } else if (stick_type == 'S') {
        //     stick_type_colored = std::string(COLOR_RED) + "S" + COLOR_RESET;
        // }
        ...
        std::cout << "  | "
                  ...
                  << " | "
                  //<< std::setw(10) << "StickType:" << std::setw(10) << std::fixed << std::setprecision(2) <<   "  "  << stick_type_colored
                  << std::endl;
    }
    else
    {
        // Key not found
        //    std::cout << "Key " <<  exchange_ordernumber1 << " not found!" << std::endl;
    }
}
    ...
    while (true) {
        ...
        while (std::getline(file, line)) {
            ...
            if (parse_line(line, epoch_time, token, price, order_traded_type, exchange_ordernumber1, exchange_ordernumber2, buysellflag)) {
                //std::cout << "Parsed successfully: Token: " << token << ", Order Type: " << order_traded_type << std::endl;
